...
import { BEAD_COLORS } from './CanonicalBoard';
...
export function GameOverSummary({ winner, history, onNewGame }: GameOverSummaryProps) {
              ...
              <tr key={moveIdx}>
                ...
                <td>
                  <div className="flex items-center gap-2">
                    <span className={`w-5 h-5 rounded-full ${BEAD_COLORS[record.beadColorIndex]} border border-white/30 shrink-0`} />
                    <SummaryBoard
                      board={record.chosenChildBoard}
                      highlightCell={record.chosenChildBoard.findIndex((c, i) => c !== record.canonicalBoard[i])}
...
function SummaryBoard({ board, highlightCell = -1, highlightColor }: { board: Board; highlightCell?: number; highlightColor?: string }) {
  return (
    <div className="grid grid-cols-3 w-24 h-24 border border-base-300 rounded shrink-0">
      {board.map((cell, i) => (
        <div key={i} className={`w-8 h-8 flex items-center justify-center text-xs font-bold border border-base-300/50 ${i === highlightCell && highlightColor ? highlightColor : ''}`}>
          {cell === 'x' && <span className={i === highlightCell ? 'text-white' : 'text-primary'}>X</span>}
